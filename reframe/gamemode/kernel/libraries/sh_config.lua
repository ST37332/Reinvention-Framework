...
function re.config.class:Create(key)
	local config = re._kernel:NewMetaTable(re.config.class);
	
	config.data = re.config.stored[key];
	config.key = key;
...
function re.config.class:IsValid()
	return self.data != nil;
end;

-- A function to query the config.
function re.config.class:Query(key, default)
...
function re.config.class:GetBoolean(default)
	if (self.data) then
...
function re.config.class:GetNumber(default)
	if (self.data) then
...
function re.config.class:GetString(default)
	if (self.data) then
...
function re.config.class:GetDefault(default)
	if (self.data) then
...
function re.config.class:GetNext(default)
...
function re.config.class:Get(default)
...
function re.config:GetStored()
	return self.stored;
end;

-- A function to import a config file.
function re.config:Import(fileName)
...
function re.config:Parse(text)
	for key in string.gmatch(text, "%$(.-)%$") do
		local value = self:Get(key):Get();
		
		if (value != nil) then
			text = re._kernel:Replace( text, "$"..key.."$", tostring(value) );
		end;
	end;
...
if (SERVER) then
	function re.config:Save(fileName, configTable)
		if (configTable) then
	...
	function re.config:Send(player, key)
		if ( player and player:IsBot() ) then
			re._kernel:Call("PlayerConfigInitialized", player);
	...
	function re.config:Load(fileName, loadGlobal)
		if (!fileName) then
	...
	function re.config:Add(key, value, isShared, isGlobal, isStatic, isPrivate, needsRestart)
	...
	function re.config.class:Set(value, map, forceSet, temporary)
		if (map) then
		...
		if ( self.data and re.config:IsValidValue(value) ) then
			if (self.data.value != value) then
				...
				if (!self.data.isStatic or forceSet) then
					if (!map or string.lower( game.GetMap() ) == map) then
						if ( (!re.config:HasInitialized() and self.data.value == self.data.default)
						or !self.data.needsRestart or forceSet ) then
							self.data.value = value;
							
							if (self.data.isShared) then
								re.config:Send(nil, self.key);
							end;
						end;
					end;
					
					if ( re.config:HasInitialized() ) then
						...
						if (!self.data.map and !self.data.temporary) then
							re.config:Save("config", re.config.stored);
						end;
						
						if (self.data.map) then
							if (default) then
								if ( re.config.map[self.data.map] ) then
									re.config.map[self.data.map][self.key] = nil;
								end;
							else
								if ( !re.config.map[self.data.map] ) then
									re.config.map[self.data.map] = {};
								end;
								
								re.config.map[self.data.map][self.key] = {
							...
							if (!self.data.temporary) then
								re.config:Save( "config/"..self.data.map, re.config.map[self.data.map] );
							end;
						end;
					end;
				end;
				
				if ( self.data.value != previousValue and re.config:HasInitialized() ) then
					re._kernel:Call("OpenAuraConfigChanged", self.key, self.data, previousValue, self.data.value);
				end;
			end;
	...
	openAura:HookDataStream("Config", function(data)
		for k, v in pairs(data) do
			if ( re.config.indexes[k] ) then
				k = re.config.indexes[k];
			end;
			
			if ( !re.config.stored[k] ) then
				re.config:Add(k, v);
			else
				re.config:Get(k):Set(v);
			end;
		end;
		
		if ( !re.config:HasInitialized() ) then
			re.config:SetInitialized(true);
			
			for k, v in pairs(re.config.stored) do
				re._kernel:Call("OpenAuraConfigInitialized", k, v.value);
			end;
	...
	function re.config:SetSentInitialized(sentInitialized)
		self.sentInitialized = sentInitialized;
	end;

	-- A function to get whether the config has sent initialized.
	function re.config:HasSentInitialized()
		return self.sentInitialized;
	end;
	
	-- A function to set a config key's aurawatch.
	function re.config:AddAuraWatch(key, help, minimum, maximum, decimals)
		self.aurawatch[key] = {
	...
	function re.config:GetAuraWatch(key)
		return self.aurawatch[key];
	end;
	
	-- A function to add a new config key.
	function re.config:Add(key, value)
		if ( self:IsValidValue(value) ) then
			if ( !self.stored[key] ) then
				self.stored[key] = {
					default = value,
	...
	-- A function to set the config's value.
	function re.config.class:Set(value)
		if (tostring(value) == "-1.#IND") then
			value = 0;
		end;
