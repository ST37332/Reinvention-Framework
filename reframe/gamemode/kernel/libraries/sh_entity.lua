...
function re.entity:RegisterSharedVars(entity, sharedVars)
...
function re.entity:IsDoor(entity)
...
function re.entity:IsDecaying(entity)
	return entity.decaying;
end;

function re.entity:IsInBox(entity, minimum, maximum)
...
function re.entity:GetPelvisPosition(entity)
	local position = entity:GetPos();
...
function re.entity:CanSeePosition(entity, position, allowance, ignoreEnts)
	local trace = {};
...
function re.entity:CanSeeNPC(entity, target, allowance, ignoreEnts)
	local trace = {};
...
function re.entity:CanSeePlayer(entity, target, allowance, ignoreEnts)
...
function re.entity:CanSeeEntity(entity, target, allowance, ignoreEnts)
	local trace = {};
...
function re.entity:SetSharedVar(entity, key, value)
	if ( IsValid(entity) ) then
		if ( entity.sharedVars and entity.sharedVars[key] ) then
			local class = re._kernel:ConvertNetworkedClass( entity.sharedVars[key] );
			
			if (class) then
				if (value == nil) then
					value = re._kernel:GetDefaultClassValue(class);
				end;
...
function re.entity:GetSharedVar(entity, key)
	if ( IsValid(entity) ) then
		if ( entity.sharedVars and entity.sharedVars[key] ) then
			local class = re._kernel:ConvertNetworkedClass( entity.sharedVars[key] );
...
function re.entity:GetPlayer(entity)
	local player = entity:GetNetworkedEntity("player");
...
function re.entity:IsInteractable(entity)
	local class = entity:GetClass();
...
function re.entity:IsPhysicsEntity(entity)
...
function re.entity:IsPodEntity(entity)
...
function re.entity:IsChairEntity(entity)
...
if (SERVER) then
	re.Entities = {}
	function re.entity:OpenDoor(entity, delay, unlock, sound, origin)
		if ( self:IsDoor(entity) ) then
			if (unlock) then
	...
	function re.entity:MakeSafe(entity, physgunProtect, toolProtect, freezeEntity)
	...
	function re.entity:StatueRagdoll(entity, forceLimit)
	...
	function re.entity:DropItemsAndCash(inventory, cash, position, entity)
	...
	function re.entity:MakeIntoRagdoll(entity, force, overrideVelocity, overrideAngles)
	...
	function re.entity:SetChairAnimations(entity)
		...
		if ( IsValid(entity) ) then
			...
			if ( IsValid(physicsObject) ) then
				if ( !physicsObject:IsPenetrating() ) then
					...
					re._kernel:CreateTimer("collision_group_"..index, 1, 1, function()
	...
	function re.entity:SetMapEntity(entity, isMapEntity)
		if (isMapEntity) then
			re.Entities[entity] = entity;
		else
			re.Entities[entity] = nil;
		end;
	end;
	
	-- A function to get whether an entity is a map entity.
	function re.entity:IsMapEntity(entity)
		if ( re.Entities[entity] ) then
			return true;
		end;
	end;
	
	-- A function to make an entity flush with the ground.
	function re.entity:MakeFlushToGround(entity, position, normal)
		entity:SetPos( position + ( entity:GetPos() - entity:NearestPoint( position - (normal * 512) ) ) );
	end;
	
	-- A function to make an entity disintegrate.
	function re.entity:Disintegrate(entity, delay, velocity, Callback)
		if (velocity) then
	...
	function re.entity:SetPlayer(entity, player)
		entity:SetNetworkedEntity("player", player);
	end;
	
	-- A function to make an entity decay.
	function re.entity:Decay(entity, seconds, Callback)
		...
		re._kernel:CreateTimer("decay_"..index, 1, 0, function()
			...
			if ( IsValid(entity) ) then
				...
				if (a <= 0) then
					...
					re._kernel:DestroyTimer("decay_"..index);
				else
	...
	function re.entity:CreateItem(player, class, position, angles)
		local itemTable = re.item:Get(class);
		local entity = ents.Create("re_item");
	...
	function re.entity:CopyOwner(entity, target)
	...
	function re.entity:BelongsToAnotherCharacter(player, entity)
		local uniqueID = self:QueryProperty(entity, "uniqueID");
		local key = self:QueryProperty(entity, "key");
	...
	function re.entity:SetPropertyVar(entity, key, value)
		if (entity.property) then entity.property[key] = value; end;
	end;
	
	-- A function to query an entity's property table.
	function re.entity:QueryProperty(entity, key, default)
	...
	function re.entity:ClearProperty(entity)
	...
	function re.entity:CalculateDoorTextPosition(door, reversed)
		local traceData = {};
