...
function re.inventory:GetDefault(player, character)
	local inventory = {};
	local default = {};
...
if (SERVER) then
	function re.inventory:Update(player, item, amount, force, noMessage)
	...
	function re.inventory:Rebuild(player)
	...
	function re.inventory:CanHaveItemUpdated(player, item, amount)
		local inventory = openAura.player:GetInventory(player);
		local itemTable = openAura.item:Get(item);
	...
	function re.inventory:HasItem(player, item, anywhere)
		local inventory = openAura.player:GetInventory(player);
		local itemTable = openAura.item:Get(item);
	...
	function re.inventory:GetMaximumWeight(player)
	...
	function re.inventory:GetWeight(player)
		local weight = 0;
	...
	function re.inventory:CanHoldWeight(player, weight)
	...
	usermessage.Hook("aura_InventoryItem", function(msg)
		...
		if (itemTable) then
			local panel = re.inventory:GetPanel();
			local item = itemTable.uniqueID;
			
			if (amount < 1) then
				re.inventory.stored[item] = nil;
			else
				re.inventory.stored[item] = amount;
			end;
	...
	function re.inventory:GetMaximumWeight()
