...
function re.item:GetBuffer()
	return self.buffer;
end;

-- A function to get all items.
function re.item:GetAll()
	return self.stored;
end;

-- A function to get a new item.
function re.item:New()
	return {};
end;

-- A function to register a new item.
function re.item:Register(itemTable)
	...
	itemTable.index = re._kernel:GetShortCRC(itemTable.uniqueID);
...
function re.item:IsWeapon(itemTable)
...
function re.item:IsBasedFrom(name, base)
	if (name) then
...
function re.item:GetWeapon(weapon, uniqueID)
	local itemTable = nil;
	local class = weapon;
...
function re.item:Merge(item, base, temporary)
...
if (SERVER) then
	function re:PlayerDestroyItem(player, itemTable) end;
	function re:PlayerUseItem(player, itemTable, itemEntity) end;

	function re:PlayerDropItem(player, itemTable, position, entity)
	...
	function re.item:Use(player, item, soundless)
		...
		if (itemTable) then
			...
			if (hasItem and hasItem > 0) then
				if (itemTable.OnUse) then
					...
					re._kernel:Call("PlayerUseItem", player, itemTable, itemEntity);
	...
	function re.item:Drop(player, item, position, soundless, noTake)
		local itemTable = self:Get(item);
		local entity = nil;
		local trace = nil;
			...
			if ( noTake or (hasItem and hasItem > 0) ) then
				...
				if (itemTable.OnDrop) then
					...
					if ( !IsValid(entity) ) then
						entity = re.entity:CreateItem(player, uniqueID, position);
					end;
					
					if ( IsValid(entity) ) then
						if (trace and trace.HitNormal) then
							re.entity:MakeFlushToGround(entity, position, trace.HitNormal);
						end;
					end;
					...
					re._kernel:Call("PlayerDropItem", player, itemTable, position, entity);
	...
	function re.item:Destroy(player, item, soundless)
		local itemTable = self:Get(item);
		...
		if (itemTable) then
			...
			if (hasItem and hasItem > 0) then
				if (itemTable.OnDestroy) then
					...
					re._kernel:Call("PlayerDestroyItem", player, itemTable);
	...
	function re.item:GetIconInfo(itemTable)
