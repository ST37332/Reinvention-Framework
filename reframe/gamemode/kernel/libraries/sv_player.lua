re.player = {};
re.player.property = {};
re.player.sharedVars = {};

-- A function to get a player's gear.
function re.player:GetGear(player, class)
...
function re.player:SetCharacterMenuOpen(player, bReset)
...
function re.player:StartSound(player, uniqueID, sound)
...
function re.player:StopSound(player, uniqueID, iFadeOut)
	if (!player.playingSounds) then
		player.playingSounds = {};
	end;
...
function re.player:CreateGear(player, class, itemTable)
...
function re.player:IsNoClipping(player)
...
function re.player:IsAdmin(player)
...
function re.player:GetAllProperty()
...
function re.player:RegisterSharedVar(name, class, playerOnly)
...
function re.player:SetAction(player, action, duration, priority, Callback)
...
function re.player:GetAction(player, percentage)
...
function re.player:CanSeeNPC(player, target, allowance, ignoreEnts)
...
function re.player:CanSeePlayer(player, target, allowance, ignoreEnts)
	if (player:GetEyeTraceNoCursor().Entity == target) then
		return true;
...
function re.player:CanSeeEntity(player, target, allowance, ignoreEnts)
	if (player:GetEyeTraceNoCursor().Entity == target) then
		return true;
	else
		local trace = {};
...
function re.player:CanSeePosition(player, position, allowance, ignoreEnts)
	local trace = {};
...
function re.player:UpdateWeaponRaised(player)
...
function re.player:GetWeaponRaised(player, getCached)
	if (getCached) then
...
function re.player:SetupRemovePropertyDelays(player)
...
function re.player:DisableProperty(player)
	local uniqueID = player:UniqueID();
...
function re.player:GivePropertyOffline(key, uniqueID, entity, networked, removeDelay)
	openAura.entity:ClearProperty(entity);
...
function re.player:TakePropertyOffline(key, uniqueID, entity)
	if (key and uniqueID) then
		local owner = player.GetByUniqueID(uniqueID);
...
function re.player:TakeProperty(player, entity)
...
function re.player:StripDefaultAmmo(player, weapon, itemTable)
	if (!itemTable) then
...
function re.player:RestoreSecondaryAmmo(player)
	if (!weapon) then
...
function re.player:RestorePrimaryAmmo(player, weapon)
	if (!weapon) then
		weapon = player:GetActiveWeapon();
	end;
...
function re.player:SaveSecondaryAmmo(player, weapon)
	if (!weapon) then
		weapon = player:GetActiveWeapon();
	end;
...
function re.player:SavePrimaryAmmo(player, weapon)
	if (!weapon) then
		weapon = player:GetActiveWeapon();
	end;
...
function re.player:GetSecondaryAmmo(player, class, bNotSpawn)
	local spawnAmmo = self:GetSpawnAmmo(player);
	local ammo = player:QueryCharacter("ammo");
...
function re.player:GetPrimaryAmmo(player, class, bNotSpawn)
	local spawnAmmo = self:GetSpawnAmmo(player);
	local ammo = player:QueryCharacter("ammo");
...
function re.player:SetSecondaryAmmo(player, class, amount)
...
function re.player:SetPrimaryAmmo(player, class, amount)
...
function re.player:TakeSecondaryAmmo(player, class)
	local ammo = player:QueryCharacter("ammo");
...
function re.player:TakePrimaryAmmo(player, class)
	local ammo = player:QueryCharacter("ammo");
...
function re.player:ConditionTimer(player, delay, Condition, Callback)
...
function re.player:EntityConditionTimer(player, target, entity, delay, distance, Condition, Callback)
...
function re.player:GetSpawnAmmo(player, ammo)
	if (ammo) then
...
function re.player:GetSpawnWeapon(player, weapon)
	if (weapon) then
...
function re.player:TakeSpawnAmmo(player, ammo, amount)
...
function re.player:GiveSpawnAmmo(player, ammo, amount)
	if ( player.spawnAmmo[ammo] ) then
		player.spawnAmmo[ammo] = player.spawnAmmo[ammo] + amount;
	else
...
function re.player:TakeSpawnWeapon(player, class)
...
function re.player:GiveSpawnWeapon(player, class, uniqueID)
...
function re.player:GiveItemWeapon(player, item)
...
function re.player:GiveSpawnItemWeapon(player, item)
	local itemTable = openAura.item:Get(item);
...
function re.player:GiveFlags(player, flags)
...
function re.player:PlaySound(player, sound)
...
function re.player:Query(player, key, default)
...
function re.player:SetSafePosition(player, position, filter)
...
function re.player:GetSafePosition(player, position, filter)
...
function re.player:ReturnProperty(player)
	local uniqueID = player:UniqueID();
	local key = player:QueryCharacter("key");
...
function re.player:TakeFlags(player, flags)
	for i = 1, string.len(flags) do
		local flag = string.sub(flags, i, i);
...
function re.player:SetMenuOpen(player, isOpen)
...
function re.player:SetInitialized(player, initialized)
	player:SetSharedVar("initialized", initialized);
end;

-- A function to check if a player has any flags.
function re.player:HasAnyFlags(player, flags, default)
...
function re.player:HasFlags(player, flags, default)
	if ( player:GetCharacter() ) then
		local playerFlags = player:QueryCharacter("flags");
...
function re.player:UseDeathCode(player, commandTable, arguments)
...
function re.player:GetDeathCode(player, authenticated)
...
function re.player:SayRadio(player, text, check, noEavesdrop)
	local eavesdroppers = {};
	local listeners = {};
	local canRadio = true;
...
function re.player:GetRealTrace(player, useFilterTrace)
...
function re.player:CreationError(player, fault)
	if (!fault) then
...
function re.player:DeleteCharacter(player, characterID)
	local character = player.characters[characterID];
...
function re.player:UseCharacter(player, characterID)
...
function re.player:GetCharacter(player)
	return player.character;
end;

-- A function to get a player's storage entity.
function re.player:GetStorageEntity(player)
...
function re.player:GetStorageTable(player)
	return player.storage;
end;

-- A function to query a player's storage.
function re.player:QueryStorage(player, key, default)
...
function re.player:CloseStorage(player, server)
	local storage = player:GetStorageTable();
...
function re.player:GetStorageWeight(player)
	if ( player:GetStorageTable() ) then
...
function re.player:OpenStorage(player, data)
	local storage = player:GetStorageTable();
	local OnClose = self:QueryStorage(player, "OnClose");
...
function re.player:UpdateStorageWeight(player, weight)
...
function re.player:CanGiveToStorage(player, item)
	local itemTable = openAura.item:Get(item);
	local entity = self:QueryStorage(player, "entity");
...
function re.player:CanTakeFromStorage(player, item)
	local itemTable = openAura.item:Get(item);
	local entity = self:QueryStorage(player, "entity");
...
function re.player:UpdateStorageForPlayer(player, item)
...
function re.player:UpdateStorageItem(player, item, amount)
...
function re.player:GetGender(player)
...
function re.player:FormatRecognisedText(player, text, ...)
	for i = 1, #arg do
...
function re.player:RestoreRecognisedName(player, target)
	local recognisedNames = player:QueryCharacter("recognisedNames");
	local key = target:QueryCharacter("key");
...
function re.player:RestoreRecognisedNames(player)
...
function re.player:SetRecognises(player, target, status, force)
	local recognisedNames = player:QueryCharacter("recognisedNames");
...
function re.player:GetPhysDesc(player)
...
function re.player:ClearRecognisedNames(player, status, isAccurate)
	if (!status) then
		local character = player:GetCharacter();
...
function re.player:ClearName(player, status, isAccurate)
	for k, v in ipairs( _player.GetAll() ) do
		if ( v:HasInitialized() ) then
...
function re.player:HolsterAll(player)
...
function re.player:SetSharedVar(player, key, value)
	if ( IsValid(player) ) then
...
function re.player:GetSharedVar(player, key)
	if ( IsValid(player) ) then
		local sharedVarData = self.sharedVars[key];
		local playerData = player.sharedVars[key];
...
function re.player:SetBanned(player, banned)
...
function re.player:SetName(player, name, saveless)
...
function re.player:GetGeneratorCount(player)
...
function re.player:GetPropertyEntities(player, class)
	local uniqueID = player:UniqueID();
	local entities = {};
	local key = player:QueryCharacter("key");
...
function re.player:GetPropertyCount(player, class)
	local uniqueID = player:UniqueID();
	local count = 0;
	local key = player:QueryCharacter("key");
...
function re.player:GetDoorCount(player)
	local uniqueID = player:UniqueID();
	local count = 0;
	local key = player:QueryCharacter("key");
...
function re.player:TakeDoorAccess(player, door)
...
function re.player:GiveDoorAccess(player, door, access)
	local key = player:QueryCharacter("key");
...
function re.player:HasDoorAccess(player, door, access, isAccurate)
	if (!access) then
...
function re.player:GetInventory(player)
...
function re.player:CinematicTextAll(text, color, hangTime)
...
function re.player:NotifyInRadius(text, class, position, radius)
	local listeners = {};
...
function re.player:NotifyAll(text, class)
	self:Notify(nil, text, true);
end;

-- A function to notify a player.
function re.player:Notify(player, text, class)
...
function re.player:SetWeapons(player, weapons, forceReturn)
...
function re.player:GetWeapons(player, keep)
	local weapons = {};
...
function re.player:GetEquippedWeight(player)
	local weight = 0;
...
function re.player:GetHolsteredWeapon(player)
	for k, v in pairs( player:GetWeapons() ) do
		local class = v:GetClass();
		local itemTable = openAura.item:GetWeapon(v);
...
function re.player:IsRagdolled(player, exception, entityless)
...
function re.player:SetUnragdollTime(player, delay)
...
function re.player:PauseUnragdollTime(player)
...
function re.player:StartUnragdollTime(player)
	if (player.unragdollPaused) then
		if ( player:IsRagdolled() ) then
...
function re.player:GetUnragdollTime(player)
...
function re.player:GetRagdollState(player)
	return player:GetSharedVar("ragdolled");
end;

-- A function to get a player's ragdoll entity.
function re.player:GetRagdollEntity(player)
...
function re.player:GetRagdollTable(player)
...
function re.player:SetRagdollImmunity(player, delay)
	if (delay) then
...
function re.player:SetRagdollState(player, state, delay, decay, force, multiplier, velocityCallback)
...
function re.player:DropWeapons(player)
...
function re.player:LightSpawn(player, weapons, ammo, forceReturn)
...
function re.player:ConvertToCamelCase(base)
	local newTable = {};
...
function re.player:CharacterScreenAdd(player, character)
	local info = {
		name = character.name,
...
function re.player:ConvertCharacterMySQL(base)
...
function re.player:GetCharacterID(player)
	local character = player:GetCharacter();
...
function re.player:LoadCharacter(player, characterID, mergeCreate, Callback, force)
	local character = {};
...
function re.player:SetBasicSharedVars(player)
...
function re.player:GetCharacterDataString(player, character, rawTable)
...
function re.player:GetCharacterRecognisedNamesString(player, character)
...
function re.player:GetCharacterInventoryString(player, character, rawTable)
...
function re.player:ConvertCharacterRecognisedNamesString(data)
...
function re.player:ConvertDataString(player, data)
	local success, value = pcall( Json.Decode, data );
	if (success) then
		return value;
	else
...
function re.player:ConvertCharacterDataString(data)
	local success, value = pcall( Json.Decode, data );
...
function re.player:SaveData(player, create, delay, bReturnQuery)
	if (create) then
...
function re.player:GetDataUpdateQuery(player)
	local schemaFolder = openAura:GetSchemaFolder();
	local steamName = tmysql.escape( player:SteamName() );
	local ipAddress = player:IPAddress();
...
function re.player:UpdateCharacter(player)
...
function re.player:SaveCharacter(player, create, character, Callback)
	if (create) then
...
function re.player:GetWeaponClass(player, safe)
	if ( IsValid( player:GetActiveWeapon() ) ) then
...
function re.player:PreserveAmmo(player)
	local weapon = player:GetActiveWeapon();
...
function re.player:GetWages(player)
